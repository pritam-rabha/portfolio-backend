...
export const submitContact = async (req, res, next) => {
  try {
    ...
    res.status(201).json({ success: true, message: 'Your message has been received. I will get back to you soon!', id: newMessage._id });
  } catch (err) { next(err); }
};
...
export const getMessages = async (req, res, next) => {
  try {
    ...
    res.status(200).json({ success: true, total, page, pages: Math.ceil(total / limit), data: messages });
  } catch (err) { next(err); }
};

export const markAsRead = async (req, res, next) => {
  try {
    const msg = await Message.findByIdAndUpdate(req.params.id, { isRead: true }, { new: true, runValidators: false });
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
          ...
          <hr style="border:none;border-top:1px solid #eee;margin:24px 0"/>
          <p style="color:#aaa;font-size:12px"><strong>Original message from ${msg.name}:</strong><br/>${msg.message.replace(/\n/g, '<br/>')}</p>
        </div>`,
    });
